...
import type { Gericht, CreateGericht } from '@/types/app';
...
export class LivingAppsService {
  // --- GERICHT ---
  static async getGericht(): Promise<Gericht[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GERICHT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Gericht[];
    return enrichLookupFields(records, 'gericht');
  }
  static async getGerichtEntry(id: string): Promise<Gericht | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GERICHT}/records/${id}`);
    const record = { record_id: data.id, ...data } as Gericht;
    return enrichLookupFields([record], 'gericht')[0];
  }
  static async createGerichtEntry(fields: CreateGericht) {
    return callApi('POST', `/apps/${APP_IDS.GERICHT}/records`, { fields: cleanFieldsForApi(fields as any, 'gericht') });
  }
  static async updateGerichtEntry(id: string, fields: Partial<CreateGericht>) {
    return callApi('PATCH', `/apps/${APP_IDS.GERICHT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'gericht') });
  }
  static async deleteGerichtEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GERICHT}/records/${id}`);
  }
