...
interface GerichtDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Gericht['fields']) => Promise<void>;
  defaultValues?: Gericht['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function GerichtDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: GerichtDialogProps) {
  const [fields, setFields] = useState<Partial<Gericht['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'gericht');
      await onSubmit(clean as Gericht['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "gericht_name": string | null, // Name des Gerichts\n  "beschreibung": string | null, // Beschreibung\n  "preis": number | null, // Preis (€)\n  "kategorie": LookupValue | null, // Kategorie (select one key: "hauptgericht" | "beilage" | "vorspeise" | "dessert" | "getraenk") mapping: hauptgericht=Hauptgericht, beilage=Beilage, vorspeise=Vorspeise, dessert=Dessert, getraenk=Getränk\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Gericht['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Gericht bearbeiten' : 'Gericht hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="gericht_name">Name des Gerichts</Label>
            <Input
              id="gericht_name"
