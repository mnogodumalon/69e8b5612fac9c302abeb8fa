...
export default function GerichtPage() {
  const [records, setRecords] = useState<Gericht[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Gericht | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Gericht | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Gericht | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getGericht());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Gericht['fields']) {
    await LivingAppsService.createGerichtEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Gericht['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateGerichtEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Gericht"
      subtitle={`${records.length} Gericht im System`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Gericht. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
