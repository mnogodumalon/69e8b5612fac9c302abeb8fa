...
import { GerichtViewDialog } from '@/components/dialogs/GerichtViewDialog';
...
const GERICHT_FIELDS = [
  ...
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'hauptgericht', label: 'Hauptgericht' }, { key: 'beilage', label: 'Beilage' }, { key: 'vorspeise', label: 'Vorspeise' }, { key: 'dessert', label: 'Dessert' }, { key: 'getraenk', label: 'Getränk' }] },
];

const ENTITY_TABS = [
  { key: 'gericht', label: 'Gericht', pascal: 'Gericht' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('gericht');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gericht': return (data as any).gericht as Gericht[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gericht': return GERICHT_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gericht': return {
        create: (fields: any) => LivingAppsService.createGerichtEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateGerichtEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteGerichtEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'gericht' || dialogState?.entity === 'gericht') && (
        <GerichtDialog
          open={createEntity === 'gericht' || dialogState?.entity === 'gericht'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'gericht' ? handleUpdate : (fields: any) => handleCreate('gericht', fields)}
          defaultValues={dialogState?.entity === 'gericht' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Gericht']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Gericht']}
        />
      )}
      {viewState?.entity === 'gericht' && (
        <GerichtViewDialog
          open={viewState?.entity === 'gericht'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'gericht', record: r }); }}
        />
      )}
